...
}; objects = MappingProxyType(temp_objects); del temp_objects

class world_object:
    def __init__(self, livestat=None, *rest):
        self.livestat = livestat; self.isEnchanted = False; self.objectName = f"{self.livestat}"
        for x in rest:
            if x != None: self.objectName = f"{self.objectName}.{x}"
        if debug: print(f"Created object: {self.objectName}")
        if self.livestat in objects:
            level = objects[self.livestat]; path = [self.livestat]
            for n in rest:
                if n is None: break
                if n in level: level = level[n]; path.append(n)
                else: uhoh = ".".join(path); raise Exception(f"Unknown subtype: \"{n}\" for object: \"{uhoh}\"")
        else: raise Exception(f"Unknown object type: \"{self.livestat}\"")
        current = objects[self.livestat]
        for m, key in enumerate(rest):
            if key is None: break
            next = current[key]
            if m + 1 == len(rest) or rest[m + 1] is None:
                if isinstance(next, (dict, list)): print(f"Warning! \"{key}\" has no further specifiers. This will select a subtype at random!")
                break
            current = next
...
def multicommand(*args, cmd, autoprint=False):
    ...
    if (autoprint or debug) and len(se) > 0: print(f"Selected objects ({se}) executed command: {cmd}")
    else: print(f"No objects could execute the command: {cmd}. Make sure they are indexed and enchanted before running multicommand().")
    return se